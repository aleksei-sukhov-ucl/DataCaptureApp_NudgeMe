...
void main() {
  ...
  testWidgets("Steps page loads correctly", (WidgetTester tester) async {
    ...
    when(mockedUserWellbeingDB.getLastWeekOfSpecificColumns(
            id: card.cardId + 3))
    ...
    expect(findWeekToggle, findsOneWidget);
    ...
    //Start date should be 6 days before today's date
    final startDate = DateFormat.yMMMd('en_US')
    ...
    final descriptionBody = find.descendant(
        of: findDescriptionCard, matching: find.byWidget(card.cardDescription));
  ...
  testWidgets("Switching to Month View", (WidgetTester tester) async {
    SharedPreferences.setMockInitialValues({
      'postcode': 'N6',
      'support_code': '12345',
      PREV_PEDOMETER_PAIR_KEY: ["0", DateTime.now().toIso8601String()]
    ...
    await withClock(
        ...
        () async => await tester.tap(findMonthToggle));
    await tester.pump();
